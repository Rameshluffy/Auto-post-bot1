...
class Database:
    ...
    def __init__(self, uri, database_name):
        ...
        self.delay = self.db.delay
        self.aforwardChnl = self.db.aforwardChnl
    ...
    async def setDelay(self, time):
        await self.delay.delete_many({})        
        await self.delay.insert_one({'time': time})

    async def findTime(self):
        doc = await self.delay.find_one()
        if doc:
            return doc['time']
        else:
            return None
    async def addAutoForwardChnl(self , sourceChnl , targetChnls):
        try :
            isAdded =await self.aforwardChnl.find_one({"sourceChnl" : sourceChnl})
            if isAdded:
                return await self.aforwardChnl.update_one({"sourceChnl" : sourceChnl} ,{'$set' : {"targetChnls" : targetChnls}})
            else:
                data = {
                "sourceChnl" : sourceChnl,
                "targetChnls" : targetChnls }
                return await self.aforwardChnl.insert_one(data)
        except Exception as e:
            traceback.print_exc()
            return False
    async def delAutoForwardChnl(self , sourceChnl):
        return await self.aforwardChnl.find_one_and_delete({"sourceChnl" : sourceChnl})
    
            
    async def getChnlLists(self , sourceChnl):
        isAdded = await self.aforwardChnl.find_one({"sourceChnl" : sourceChnl})
        if isAdded:
            return isAdded['targetChnls']
        else :return False
