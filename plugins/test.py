...
class CLIENT:
    ...
    async def add_bot(self, bot, message):
        ...
        msg = await bot.listen(user_id)
        if msg:
            newMsg = await msg.reply("<b>Wait Im Working...</b>")
        if msg.text == "/cancel":

            await newMsg.edit("Process Cancelled !")
            return
        elif not msg.forward_date:
            await newMsg.edit("This Is Not A Forward Message")
            return
        elif str(msg.forward_from.id) != "93372553":
            await newMsg.edit("This Message Was Not Forward From Bot Father")
            return
        bot_token = re.findall(
            r"\d[0-9]{8,10}:[0-9A-Za-z_-]{35}", msg.text, re.IGNORECASE
        )
        bot_token = bot_token[0] if bot_token else None
        if not bot_token:
            await newMsg.edit("There Is No Bot Token In That Message")
            return
        try:
            _client = await start_clone_bot(self.client(bot_token, False), True)
        except Exception as e:
            await newMsg.edit(f"Bot Error :</b> `{e}`")
        _bot = _client.me
        details = {
            "id": _bot.id,
            "is_bot": True,
