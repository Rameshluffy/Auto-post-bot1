...
async def settings(client, message):
    ...
    await message.reply_photo(
        photo=Config.START_PIC, caption=text, reply_markup=main_buttons(), quote=True
    )
...
async def settings_query(bot, query):
    ...
    if type == "main":
        await query.message.edit_text(
            "<b>Change Your Settings As Your Wish</b>", reply_markup=main_buttons()
        )
        ...
        if _bot is not None:
            buttons.append(
                [InlineKeyboardButton(_bot["name"], callback_data=f"settings#editbot")]
            )
        else:
            buttons.append(
            ...
            buttons.append(
                [
                    InlineKeyboardButton(
                        "✚ Add User Bot ✚", callback_data="settings#adduserbot"
                    )
                ]
            )
        ...
        await query.message.edit(BOT_TOKEN_TEXT)
        getVal = await CLIENT.add_bot(bot, query)
        if bot:
            await getVal.edit(
            ...
            await bot.edit_message_media(
                query.message.chat.id, getVal.id, InputMediaPhoto(Config.START_PIC)
            )
        ...
        buttons.append(
            [
                InlineKeyboardButton(
                    "✚ Add Channel ✚", callback_data="settings#addchannel"
                )
            ]
        )
        buttons.append([InlineKeyboardButton("🔙 Back", callback_data="settings#main")])
        ...
        try:
            text = await bot.send_message(
                user_id,
                "<b><u>Set Target Chat</u></b>\n\nForward A Message From Your Target Chat\n/cancel - To Cancel This Process",
            )
            chat_ids = await bot.listen(chat_id=user_id, timeout=300)
            if chat_ids.text == "/cancel":
                await chat_ids.delete()
                return await text.edit_text(
                    "Process Canceled", reply_markup=InlineKeyboardMarkup(buttons)
                )
        ...
        await query.message.edit_text(
            "Successfully Updated", reply_markup=InlineKeyboardMarkup(buttons)
        )
        ...
        buttons = [
            [
                InlineKeyboardButton(
                    "❌ Remove ❌", callback_data=f"settings#removechannel_{chat_id}"
                )
            ],
        ...
        if caption is None:
            buttons.append(
                [
                    InlineKeyboardButton(
                        "✚ Add Caption ✚", callback_data="settings#addcaption"
                    )
                ]
            )
        else:
            buttons.append(
                [
                    InlineKeyboardButton(
                        "👀 See Caption", callback_data="settings#seecaption"
                    )
                ]
            )
            buttons[-1].append(
                InlineKeyboardButton(
                    "🗑️ Delete Caption", callback_data="settings#deletecaption"
                )
            )
        buttons.append([InlineKeyboardButton("🔙 Back", callback_data="settings#main")])
        await query.message.edit_text(
        ...
        buttons = [
            [
                InlineKeyboardButton(
                    "✏️ Edit Caption", callback_data="settings#addcaption"
                )
            ],
        ...
        if button is None:
            buttons.append(
                [
                    InlineKeyboardButton(
                        "✚ Add Button ✚", callback_data="settings#addbutton"
                    )
                ]
            )
        else:
            buttons.append(
                [
                    InlineKeyboardButton(
                        "👀 See Button", callback_data="settings#seebutton"
                    )
                ]
            )
            buttons[-1].append(
                InlineKeyboardButton(
                    "🗑️ Remove Button ", callback_data="settings#deletebutton"
                )
            )
        buttons.append([InlineKeyboardButton("🔙 Back", callback_data="settings#main")])
        await query.message.edit_text(
            "<b><u>Custom Button</b></u>\n\nYou Can Set A Inline Button To Messages.\n\n<b><u>Format :</b></u>\n`[☉ SB Botz Update ☉][buttonurl:https://t.me/SB_Botz_Update]`\n",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
        ...
        try:
            txt = await bot.send_message(
                user_id,
                text="**Send your custom button.\n\nFORMAT:**\n`[forward bot][buttonurl:https://t.me/Sbforwardbot]`\n",
            )
            ...
            await ask.delete()
            await txt.edit_text(
                "Successfully Button Added", reply_markup=InlineKeyboardMarkup(buttons)
            )
        except asyncio.exceptions.TimeoutError:
            await txt.edit_text(
                "Process Has Been Automatically Cancelled",
        ...
        await query.message.edit_text(
            "**Your Custom Button**", reply_markup=InlineKeyboardMarkup(button)
        )

    elif type == "deletebutton":
        await update_configs(user_id, "button", None)
        await query.message.edit_text(
            "Successfully Button Deleted", reply_markup=InlineKeyboardMarkup(buttons)
        )
        ...
        if db_uri is None:
            ...
            buttons[-1].append(
                InlineKeyboardButton("🗑️ Remove URL", callback_data="settings#deleteurl")
            )
        buttons.append([InlineKeyboardButton("🔙 Back", callback_data="settings#main")])
        await query.message.edit_text(
        ...
        uri = await bot.ask(
            user_id,
            "<b>please send your mongodb url.</b>\n\n<i>get your Mongodb url from [here](https://mongodb.com)</i>",
            disable_web_page_preview=True,
        )
        if uri.text == "/cancel":
            return await uri.reply_text(
                "Process Cancelled !", reply_markup=InlineKeyboardMarkup(buttons)
            )
        if not uri.text.startswith("mongodb+srv://") and not uri.text.endswith(
            "majority"
        ):
            return await uri.reply(
                "Invalid Mongodb URL", reply_markup=InlineKeyboardMarkup(buttons)
            )
        await update_configs(user_id, "db_uri", uri.text)
        await uri.reply(
            "Successfully Database URL Added ✅",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
        ...
        await query.message.delete()
        ext = await bot.ask(
            user_id, text="Please Send Your Extensions (Seperete By Space)"
        )
        if ext.text == "/cancel":
        ...
        await ext.reply_text(
            f"Successfully Updated", reply_markup=InlineKeyboardMarkup(buttons)
        )
        ...
        await query.message.edit_text(
            text="Successfully Deleted", reply_markup=InlineKeyboardMarkup(buttons)
        )
    elif type == "add_keyword":
        await query.message.delete()
        ask = await bot.ask(
            user_id, text="Please Send The Keywords (Seperete By Space)"
        )
        if ask.text == "/cancel":
...
def main_buttons():
    buttons = [
        ...
        [
            InlineKeyboardButton(
                "⚙️ Extra Settings", callback_data="settings#nextfilters"
            )
        ],
...
def size_button(size):
    buttons = [
        ...
        [
            InlineKeyboardButton(
                "+1", callback_data=f"settings#update_size-{size + 1}"
            ),
            InlineKeyboardButton(
                "-1", callback_data=f"settings#update_size_-{size - 1}"
            ),
        ],
        [
            InlineKeyboardButton(
                "+5", callback_data=f"settings#update_size-{size + 5}"
            ),
            InlineKeyboardButton(
                "-5", callback_data=f"settings#update_size_-{size - 5}"
            ),
        ],
        [
            InlineKeyboardButton(
                "+10", callback_data=f"settings#update_size-{size + 10}"
            ),
            InlineKeyboardButton(
                "-10", callback_data=f"settings#update_size_-{size - 10}"
            ),
        ],
        [
            InlineKeyboardButton(
                "+50", callback_data=f"settings#update_size-{size + 50}"
            ),
            InlineKeyboardButton(
                "-50", callback_data=f"settings#update_size_-{size - 50}"
            ),
        ],
        [
            InlineKeyboardButton(
                "+100", callback_data=f"settings#update_size-{size + 100}"
            ),
            InlineKeyboardButton(
                "-100", callback_data=f"settings#update_size_-{size - 100}"
            ),
        ],
